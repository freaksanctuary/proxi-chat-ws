...
const messages = new Elysia({ prefix: "/messages" }).use(authMiddleware).get(
  "/",
  async ({ auth }) => {
    const messagesStr =
      (await redisClient.lRange(`messages:${auth.roomId}`, 0, -1)) ?? [];
    const messages: Message[] = messagesStr.map(
